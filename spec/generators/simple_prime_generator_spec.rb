...
describe PrimeNumbers::SimplePrimeGenerator do
  describe 'is_prime?' do
    it 'returns false when given a negative number' do
      (-20..-1).each do |number|
        expect(PrimeNumbers::SimplePrimeGenerator.is_prime? number).to be false
      end
    end

    it 'returns true when given prime numbers' do
      PRIME_NUMBERS.each do |number|
        expect(PrimeNumbers::SimplePrimeGenerator.is_prime? number).to be true
      end
    end
  ...
  describe 'generate' do
    it 'should generate the requested number of primes' do
      expect(PrimeNumbers::SimplePrimeGenerator.generate 5).to eq PRIME_NUMBERS.first(5)
      expect(PrimeNumbers::SimplePrimeGenerator.generate 10).to eq PRIME_NUMBERS.first(10)
    end
  end
end